...
ShinySetup=function(path=getwd()){
  ...
  for (i in 1:length(folders)){
    if(dir.exists(paste0(path,"/",folders[i],sep=""))){cat(crayon::red(cli::symbol$cross)," Folder",paste0("~/",folders[i],sep="")," already exits...\n Skipping this folder...\n")
    }else{
...
\n library(shiny) \n #add additional packages required here...."
  ...
  for (i in 1:length(Scripts)) {
    ...
    if(file.exists(filename)){cat(crayon::red(cli::symbol$cross)," ",paste0(Scripts[i],".R",sep=""),"already exits...\n Skipping this file...\n")
    }else{
